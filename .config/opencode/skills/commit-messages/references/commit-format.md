...
**Keywords:** `Fixes #N`, `Closes #N`, `Resolves #N` (GitHub/GitLab),
`PROJ-123` (Jira)
...
All other rules still apply (imperative, 72-char limit includes prefix, body
explains WHY).
...
- **Capitalize after colon** — `feat: Add auth` not `feat: add auth` (unless
  project uses lowercase)
